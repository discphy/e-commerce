...
class UserCouponControllerE2ETest extends E2EControllerTestSupport {
    ...
    @Test
    void publishCouponWithInvalidStatus() {
        // given
        Coupon coupon = Coupon.create("쿠폰명1", 0.1, 10, CouponStatus.REGISTERED, LocalDateTime.now().plusDays(1));
        ...
        .then()
            ...
            .body("message", equalTo("쿠폰을 발급할 수 없습니다."));
    }

    @DisplayName("쿠폰 발급 시, 만료된 쿠폰은 발급할 수 없다.")
    @Test
    void publishCouponWithExpiredDate() {
        // given
        ...
        .then()
            ...
            .statusCode(HttpStatus.INTERNAL_SERVER_ERROR.value())
            .body("code", equalTo(500))
            .body("message", equalTo("쿠폰이 만료되었습니다."));
    }
    ...
    void publishCouponWithInsufficientQuantity() {
        // given
        Coupon coupon = Coupon.create("쿠폰명1", 0.1, 0, CouponStatus.PUBLISHABLE, LocalDateTime.now().plusDays(1));
        couponRepository.save(coupon);
        ...
        .then()
            ...
            .statusCode(HttpStatus.INTERNAL_SERVER_ERROR.value())
            .body("code", equalTo(500))
            .body("message", equalTo("쿠폰 수량이 부족합니다."));
    }
